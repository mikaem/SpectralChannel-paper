...
\title{A Shen-Fourier turbulent channel flow solver for large-scale simulations}
...
In his seminal papers \cite{Shen94,Shen95}, Jie Shen describe how to construct Legendre and Chebyshev basis functions that lead to sparse matrices, susceptible to fast ($\mathcal{O}(N)$) direct solvers. To the authors knowledge, the bases have not been used for large-scale channel flow simulations, and algorithms for the required direct solvers have not, until now, been devised. Shen's bases have been used for the Navier Stokes equations before, though. Bouchon et.al.  \cite{Bouchon01} describe a spectral-Galerkin formulation very similar to the one used in this paper. However, they choose the Legendre basis over the Chebyshev, neglecting the higher cost of transforms and arguing instead for the simpler structure of the matrices and the lack of weight functions for the Legendre basis. The cost of the Legendre transforms, though, makes the basis unsuitable for larger scales, and the simulations performed in \cite{Bouchon01} were rather small.
...
\partial_x v)$, is computed by projecting each term to its appropriate space. 
To this end the two terms $\partial_x v$ and $\partial_x w$ are projected to 
$V_N$, whereas the remaining $\partial_y u$ and $\partial_z u$ are projected to 
$\N{V}_N$. Note that the nonlinear term is also generally in need of 
...
pseudocode conventions of Kopriva \cite{kopriva09}, with one minor difference. 
A statement like $\{w\}_{k=0}^{N} \gets \{u\}_{k=0}^N$ will here be used to 
represent a 
vectorization, where components $u_k$, for $k=0,1,\ldots,N$, are copied from 
$u_k$ to $w_k$. Similar conventions apply to matrices, e.g., 
$\{U_{1,k}\}_{k=0}^N = \{V_{2,k}\}_{k=0}^N$ can be used for copying row 2 of 
$V$ to row 1 of $U$.} Note 
...
a proper initialization would need to set both the two first time steps.
\begin{algorithm}
...
the 3/2-rule \cite{canuto1988} or the 2/3-rule \cite{Orzag71}. The 
transforms are carried out using fast Fourier and fast Chebyshev transforms 
that are all of order $\mathcal{O}(N_x\log N_x)$ per one-dimensional transform 
of size $N_x$. The Shen transforms make use of the Chebyshev transforms that in 
turn are using 
...
\begin{algorithm}
	...
	(\ref{eq:rk}) respectively. \label{alg:ab}}
		\Fn{\reduceAB{$ \{\N{L}_{kj}\}_{k,j=0}^{N_x-4}, \,\xi_0$}}{
        \Output{$\{a_k\}_{k=0}^{N_x-4}, \{b_k\}_{k=0}^{N_x-4}$}  \;
		\For{$k=0,1$}{
		     $a_k \gets p_k$ \;
		     $b_k \gets r_k$ \;
		}
		\For{$k=2,3$}{
		    $a_k \gets p_k - \N{L}_{k,k-2}\,a_{k-2}$ \;
		    $b_k \gets r_k - \N{L}_{k, k-2}\,b_{k-2}$ \;
		}
		\For{$k=4$ \KwTo $N_x-4$}{
		...
		\Return $\{a_k\}_{k=0}^{N_x-4}, \{b_k\}_{k=0}^{N_x-4}$ \;
    }
\end{algorithm}
...
\begin{algorithm}
	...
	$\{\N{U}_{kj}\}_{k,j=0}^{N}, 
		\{\N{L}_{kj}\}_{k,j=0}^{N}$ 
	matrices, as well as the $\{ a_k\}_{k=0}^N$ and $\{ b_k\}_{k=0}^N$ vectors. 
	...
	(\ref{eq:rk}) respectively. 	\label{alg:SolveBiharmonic}}
		\Fn{\LUSolveBiharmonic{$\{\N{U}_{kj}\}_{k,j=0}^{N}, 
		  ...
		  $y_0 \gets f_0$ \;
		  $y_1 \gets f_1$ \;
		  $y_2 \gets f_2 - \N{L}_{20}y_{0}$ \;
		  $y_3 \gets f_3 - \N{L}_{31}y_{1}$ \;
		  \For{$k = 4$ \KwTo $N$}{
		  $y_k \gets f_k - \N{L}_{k,k-2}y_{k-2} - \N{L}_{k, k-4}y_{k-4}$ \;
		  }
		  \tcp*[l]{Solve $\N{U}_{kj}u_j=y_k$ with back 
              substitution}                    
		  $\hat{u}_N \gets y_N / \N{U}_{NN}$ \;
	      $\hat{u}_{N-1} \gets y_{N-1} / \N{U}_{N-1,N-1}$ \;
	      $\hat{u}_{N-2} \gets \left(y_{N-2} - \N{U}_{N-2, 
	      N}y_N\right)/\N{U}_{N-2,N-2}$ \;
	      $\hat{u}_{N-3} \gets \left(y_{N-3} - \N{U}_{N-3, 
...
The complete Navier Stokes solver should scale like $\mathcal{O}(N)$ and 
$\mathcal{O}N\log N)$ since it is a combination of linear algebra routines 
$\mathcal{O}(N)$, fast transforms $\mathcal{O}(N \log N)$ and several 
elementwise array operations $\mathcal{O}(N)$. 

We show in Table \ref{tab:timings_solver} some timings for one single time 
step, increasing only the wall normal discretization. We show that the total 
time scales approximately as $N \log N$ for large $N$ and better than linearly 
for small $N$. This shift happens since at small $N$ the transforms are fast 
and linear operations, like elementwise multiplications, are more dominating. 
We also see that the time for assembly of the right hand side is approximately 
$\mathcal{O}(N \log N)$, whereas the time for solving the linear systems is 
...
Timings in Table \ref{tab:timings_solver} are computed with dealiasing 
...
\begin{table}
	...
	\begin{tabular}{ccccc}	
		...
		\hline
4	&	1.23454361e-02	&	2.44150583e-04	&	1.23454363e-02	&	
2.44150404e-04	\\
5	&	1.15753545e-02	&	2.44052002e-04	&	1.15753547e-02	&	
2.44051822e-04	\\
6	&	1.15673948e-02	&	2.44051018e-04	&	1.15673950e-02	&	
2.44050836e-04	\\
7	&	1.15673151e-02	&	2.44051004e-04	&	1.15673154e-02	&	
2.44050816e-04	\\
8	&	1.15673142e-02	&	2.44050426e-04	&	1.15673156e-02	&	
2.44050992e-04	\\
9	&	1.15673192e-02	&	2.44049842e-04	&	1.15673185e-02	&	
2.44050701e-04	\\
10	&	1.15672575e-02	&	2.44043530e-04	&	1.15674820e-02	&	
2.44052242e-04	\\
11	&	1.15649705e-02	&	2.43713173e-04	&	1.15684397e-02	&	
2.44010407e-04	\\
12	&	1.15568665e-02	&	2.43541116e-04	&	1.15844457e-02	&	
2.44202451e-04
	\end{tabular}
\end{table}
...
Simulations of turbulent flows. A spectral discretization of the wall normal 
direction in a plane channel is challenging due to roundoff errors from poorly 
conditions coefficient matrices, and as such all recent large-scale channel DNS 
have been conducted without spectral accuracy in the wall normal direction. 

In this paper a Navier Stokes solver for large-scale turbulent channel flows 
...
$\mathcal{O}(N \log N)$, for a mesh of size $N \times N \times N$, due to fast 
transforms. Direct 
solvers have been devised for Helmholtz and biharmonic coefficient matrices, 
and these have been shown to scale close to 
$\mathcal{O}(N)$, with insignificant roundoff errors. The fully spectral solver 
has been applied to a turbulent 
channel flow at $Re_{\tau}=2000$, and the first order statistics are shown to agree well with the literature.

\section*{Acknowledgements}
This research is a part of the 4DSpace Strategic Research Initiative at the 
University of Oslo.
...
\begin{algorithm}
	...
	\D{V}_N, \N{V}_N$. 
		Here "dct" is the discrete cosine transform from SciPy. \label{alg:fst}}
			...
			$\{ w_k\}_{k=0}^{N} \gets \text{dct}(\bm{f}, \text{type=2}, 
			\text{axis=0})\frac{\pi}{2 N}$ \;
            }
			\ElseIf{ points = "Gauss-Chebyshev"}{
			$\{ w_k\}_{k=0}^{N} \gets \text{dct}(f, \text{type=1}, 
...
\begin{algorithm}
	\caption{Inverse transforms for all spaces $V_N, \D{V}_N, \N{V}_N$. 
		...
		\Fn {\inverseChebTransform{$\{f\}_{j=0}^N$, points}}{
        \Output{$\{y_k\}_{k=0}^{N}$}    
        \If{points = "Gauss-Chebyshev"}{
		    $\{y_k\}_{k=0}^{N} \gets \text{dct}(\bm{f}, \text{type=3}, 
		    \text{axis=0})$ \;
		    $\{y_k\}_{k=0}^{N} \gets \frac{1}{2}\{y_k\}_{k=0}^{N} + 
		    \frac{1}{2}f_0$ \;
		}
        \ElseIf{ points = "Gauss-Lobatto"}{
	        $\{y_k\}_{k=0}^{N} \gets \text{dct}(\bm{f}, \text{type=1}, 
	        \text{axis=0})$ \;
	        $\{y_k\}_{k=0}^{N} \gets \frac{1}{2}\{y_k\}_{k=0}^{N} + 
	        \frac{1}{2}f_0$ \;
	        \For{$k=0$ \KwTo $N$}{
	         $y_k \gets y_k + \frac{(-1)^k}{2} f_N$ \;
    	    }
	    }
	    \Return{$\{y_k\}_{k=0}^{N}$}
    }
	    
	    \Fn{\inverseShenTransform{$\{f_j\}_{j=0}^{N}$, points, space}}{
		\Output{$\{y_k\}_{k=0}^{N}$}
        $\{z_k\}_{k=0}^{N} \gets 0$ \;
		\If{ space = $\D{V}_N$}{
        ...
        $\{y_k\}_{k=0}^{N} \gets $ inverseChebTransform($\{z_k\}_{k=0}^N$, 
        points) 
        \;
		\Return $\{y_k\}_{k=0}^{N}$ \;
    }
