...
in Sec.~\ref{sec:discretizationNS}, is given in Algorithm \ref{alg:NS}. Note 
...
a proper initialization would need to set both the two first time steps instead.
\begin{algorithm}
...
%			\State $\{y_k\}_{k=0}^{N} \gets$ forwardScalar($\{f_j\}_{j=0}^{N}$, 
%points, space)
%			\If{space = $\D{V}_N$}
%			\State $B \gets \D{B}$
%			\ElsIf{space = $\N{V}_N$}
